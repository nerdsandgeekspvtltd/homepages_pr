...
namespace AssociationRepository.Association
{
    ...
    public class BlogPagesRepository : IBlogPagesRepository
    {
        ...
        public async Task<BlogPostResponse> GetAllBlogPages(BlogFilters blogFilters)
        {
            ...
            var data = _associationContext.BlogPosts.Where(x => x.ScopeType == "public" && (x.ValidFrom == null || (x.ValidFrom != null && x.ValidFrom >= DateTime.Now)) && (x.ValidTo == null || (x.ValidTo != null && x.ValidTo <= DateTime.Now))).Include(x => x.BlogAttributes).Include(x => x.Tenant).AsQueryable();

            if (blogFilters.TenantId > 0)
            {
                data = data.Where(e => e.TenantId == blogFilters.TenantId);
            }
            ...
            {
                data = data.Where(e => e.Title.Contains(blogFilters.Keyword));
            }
    ...
    (blogFilters.CreatorIds != null && blogFilters.CreatorIds.Any()))
            ...
            blogPostResponse.blogPosts = await data.ToListAsync();

            blogPostResponse.MaximumCount = blogPostResponse.blogPosts.Count();

            if (blogFilters.Page > 0 && blogFilters.PageSize > 0)
            {
                blogPostResponse.blogPosts = blogPostResponse.blogPosts.Skip((blogFilters.Page.Value - 1) * blogFilters.PageSize.Value).Take(blogFilters.PageSize.Value).ToList();
            }


            blogPostResponse.Page = blogFilters.Page;
            blogPostResponse.PageSize = blogFilters.PageSize;
        ...
        public BlogPost GetBlogPageById(int Id)
        {
            ...
            var data = _associationContext.BlogPosts.Where(x => x.Bpid == Id).Include(x => x.BlogAttributes).Include(x => x.Tenant).FirstOrDefault();
            return data;
        }
        /// <summary>
