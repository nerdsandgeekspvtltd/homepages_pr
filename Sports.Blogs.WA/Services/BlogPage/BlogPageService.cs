...
namespace Sports.Blogs.WA.Services.BlogPage
{
    public class BlogPageService : IBlogPageService
    {
        ...
        public async Task<List<BlogFilterSuggestions>> GetBlogFilterSuggestions(int TenantId)
        {
            // Send HTTP GET request with the constructed query string to the specified API endpoint
            return await _httpClient.GetFromJsonAsync<List<BlogFilterSuggestions>>("BlogPage/GetBlogFilterSuggestions/" + TenantId) ?? new();
        }
        ...
        public async Task<BlogPost> GetBlogPageById(int Id)
        {
            ...
            {
                // Send HTTP GET request with the constructed query string to the specified API endpoint
                blogPost = await _httpClient.GetFromJsonAsync<BlogPost>("BlogPage/GetBlogPageById/" + Id) ?? new();
            }
            catch (Exception ex)
            {
                blogPost = new();
